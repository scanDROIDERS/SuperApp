...
import com.nnt.myapplication.databinding.ActivityDialogGetBinding
...
class activity_getactivity : AppCompatActivity() {
    ...
    fun calcularTMB(idade: Int, altura: Int, peso: Double) {
        ...
        when {
            binding.chipMasculino.isChecked -> {
                ...
                when {
                    binding.chipSedentario.isChecked -> {
                        get = tmb * 1.2
                    }
                    binding.chipLevemente.isChecked -> {
                        get = tmb * 1.375
                    }
                    binding.chipModeradamente.isChecked -> {
                        get = tmb * 1.55
                    }
                    binding.chipMuito.isChecked -> {
                        get = tmb * 1.725
                    }
                    binding.chipExtremamente.isChecked -> {
                        get = tmb * 2
                    }
                }
                dialogTMB(get)
    ...
    fun dialogTMB(tmb: Double) {
        ...
        val dialogBinding = ActivityDialogGetBinding.inflate(LayoutInflater.from(this))
    ...
    fun infoTMB() {
        val builder = AlertDialog.Builder(this, R.style.Theme_Dialog)
        val dialogBinding = ActivityDialogInfoGetBinding.inflate(LayoutInflater.from(this))
        builder.setView(dialogBinding.root)
